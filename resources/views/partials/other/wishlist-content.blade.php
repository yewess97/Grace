        ...
        @foreach ($user_wishlist_items as $wishlist_item)
            <li role="listitem" id="item_{{$wishlist_item->id}}" class="product position-relative row justify-content-between align-items-center border-bottom">
                ...
                <article class="product-total-price d-flex justify-content-end">
                    <p class="new-price fs-6 fw-600 lh-1">@priceFormat($product->new_price)</p>
                    @oldprice ($product->old_price, $product->new_price)
                        <p class="old-price fs-7 text-decoration-line-through lh-1">@priceFormat($product->old_price)</p>
                    @endoldprice
                </article>
            </li>
        @endforeach
